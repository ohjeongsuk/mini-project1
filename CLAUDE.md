...
(한때 일별 지출 추이 선·막대 그래프가 더 있었으나 제거했다 — 아래 참조.)

| 차트 | 구현 방식 | 라이브러리가 불필요한 이유 |
|---|---|---|
| 카테고리별 지출 막대 | `div` 너비를 `%`로 | 축·툴팁·범례가 전부 불필요 |
| 예산 소진율 막대 | 〃 | 〃 |
| 일별 수입·지출 캘린더 | CSS `grid-cols-7` + 배경색 단계 | Recharts에 애초에 이런 차트가 없다 |
...
- **일별 캘린더는 칸마다 그날의 수입·지출을 적고, 누르면 그날 내역으로 간다.**
  주소는 `/transactions?from={date}&to={date}` 다 — 내역 화면이 이미 `from`·`to` 를 URL 에서
  읽으므로 API 도 필터 UI 도 손대지 않는다. **라우트는 차트가 정하지 않고 `hrefFor` 로 받는다.**

  ⚠️ **0 원인 줄은 그리지 않는다.** 한때 `0원` 을 적었지만, 칸이 좁아 값이 없는 날의 두 줄이
  값이 있는 날을 눈에서 밀어냈다. "기록이 없다" 와 "0원" 의 구분은 칸의 `title`·`aria-label`
  이 맡는다 — 그쪽은 항상 원 단위 전체 금액을 준다.

  ⚠️ **`원` 은 화면 폭과 무관하게 항상 붙인다.** 숫자만 떠 있으면 금액으로 읽히지 않는다.
  좁은 화면은 `원` 을 떼는 대신 **글자를 줄여 자리를 만든다** — 320px 7px · xs(360~) 8px ·
  sm(640~) 15px 세 단계다. 기준은 부호까지 붙인 최악값(`-10.7만원` 7자)이 칸 안쪽에 들어가는가다.
  값은 `rem` 으로 적어 사용자가 브라우저 기본 글꼴을 키우면 함께 커진다.

  ⚠️ **금액 글자는 잉크색(`text-foreground`)이고, 방향은 부호(`+`/`-`)가 말한다.**
  §8 의 "수입 초록 · 지출 빨강" 이 이 칸에서만 적용되지 않는다 — 배경 농도가 칸마다 달라
  초록 2.0:1 · 빨강 3.0:1 로 본문 기준(4.5:1)에 한참 못 미쳤다. 잉크색은 같은 자리에서 14.9:1 이다.
  **목록·요약 카드의 금액은 그대로 초록·빨강이다.** 그쪽은 흰 카드 배경이라 색이 제 대비를 낸다.

  ⚠️ **배경 농도 상한은 0.15 이고 색은 전용 토큰 `--color-heat`(라벤더) 다.**
  상한을 올리면 같은 칸에 얹힌 금액이 먼저 죽는다. 농도는 "어느 날이 무거웠나" 를 훑는 용도이고
  크기는 숫자가 말한다. 색을 따로 둔 이유는 §8 참조.

  ⚠️ **다크 모드에서는 농도에 `--heat-scale` 배수(1.5)를 곱한다.** 농도 단계가 밝은 배경 기준이라
  어두운 배경에 그대로 깔면 묻힌다 — 15% 로는 순백을 깔아도 `rgb(58,58,58)` 까지밖에 못 간다.
  `color-mix()` 의 퍼센트 자리에 `calc(… * var(--heat-scale))` 을 넣어 **JS 는 "얼마나 무거운가" 만
  계산하고 "이 테마에서 얼마나 진하게 칠할지" 는 CSS 가 정하게** 한다.

  ⚠️ **격자선과 일자 라벨은 `--grid` / `--grid-label` 을 쓴다.** `--border` / `--muted-foreground`
  를 그대로 쓰지 않는다. 자세한 이유는 §8 참조.
- **일별 지출 추이 그래프는 제거했다.** 선·막대·누적을 고르던 화면과 `ChartAxis`·`TrendLine`·
  `DailyBar`·`DailyTrendChart` 네 컴포넌트, `localStorage` 의 `moneylog_chart` 키가 함께 사라졌다.
  같은 달의 일별 흐름을 캘린더가 더 잘 보여주고, 대시보드에서 같은 정보를 두 번 그릴 이유가 없었다.
  **다시 넣지 않는다.** 필요해지면 캘린더를 키우는 쪽을 먼저 검토한다.
...
- **의도 해석은 규칙 기반이다. LLM을 쓰지 않는다.** 새 의존성·API 키·호출 비용이 없고, 동작을 단위 테스트로 고정할 수 있다. LLM 리포트는 `PRD.md` 9장 2번에 별도 항목으로 남아 있다.

> ⚠️ **의도를 못 찾아도 4xx가 아니라 200 + `intent: "UNKNOWN"`이다.**
> 사용자가 예상 밖으로 물어본 것은 클라이언트 잘못도 서버 오류도 아니다.
> 4xx로 만들면 프론트의 에러 경로를 타서 토스트가 뜨고, 정작 보여줘야 할 `suggestions`를 못 보여준다.
>
> ⚠️ **조회 전용이다.** 거래 생성·수정·삭제를 이 엔드포인트로 하지 않는다.
> 자연어 오해로 데이터가 바뀌면 되돌릴 방법이 없다.
>
> ⚠️ **월 표기(`"12월"`)의 연도를 추측하지 않는다.** `asOf`가 2026-09여도 `"12월"`은 **2026-12**다.
> "미래니까 작년이겠지" 같은 보정은 사용자가 실제로 미래 달을 물었을 때 말없이 다른 달을 조회하고,
> 그 사실이 화면에 드러나지 않는다.
>
> ⚠️ **답변 문장을 프론트에서 다시 만들지 않는다.** §13의 "예측 수식은 백엔드에만"과 같은 이유다.
> 문장이 두 곳에 있으면 갈라지고, 프론트에는 그것을 검증할 테스트 러너가 없다.
...
| `/chat` | 챗봇 전체 화면 (§5 조회 전용) | O |

- 미인증 상태로 보호된 경로 접근 시 `/login`으로 리다이렉트.
- 로그인 직후 기본 진입은 **`/dashboard`**.
- 인증 화면의 공통 헤더에는 **닉네임, 네비게이션, 로그아웃 버튼**을 둔다.

### 떠 있는 챗봇 버튼 (ChatWidget)

`(main)` 레이아웃에 두어 **모든 인증 화면에 같은 자리로 떠 있다.** 대화 UI 는 `/chat` 과
`ChatPanel` 을 공유하고, 껍데기(버튼·헤더·크기)만 위젯이 갖는다.

- **`/chat` 에서는 렌더하지 않는다.** 같은 대화를 전체 화면으로 보고 있는데 입구가 둘일 이유가 없다.
- **끌어서 옮길 수 있다.** 오른쪽 아래는 저장 버튼이나 목록 끝과 자주 겹치는 자리라,
  가리면 사용자가 치울 수 있어야 한다. Pointer Events 하나로 마우스·터치·펜을 같은 코드가 처리한다.
  > ⚠️ **`setPointerCapture` 가 없으면** 포인터가 버튼 밖으로 나가는 순간 `move` 가 끊겨 버튼이 손을 놓친다.
  > ⚠️ **`touch-action: none` 이 없으면** 터치로 끌 때 브라우저가 페이지 스크롤로 해석한다.
  > ⚠️ **드래그 임계값(5px)을 0 으로 두지 않는다.** 손가락은 누르는 동안 1~3px 흔들려서,
  > 임계값이 없으면 모든 탭이 드래그로 잡혀 **챗봇이 영영 열리지 않는다.**
- **위치를 저장하지 않는다.** 새로고침하면 늘 기본 자리로 돌아온다. 테마(`moneylog_theme`)와
  다른 판단인데, 그쪽은 "어떻게 볼지" 라는 취향이고 화면 어디서나 뜻이 같은 반면, **버튼 위치는
  그 순간 가린 것을 잠깐 치우는 동작**이기 때문이다. 치운 자리가 다음 화면에서는 오히려 방해가
  될 수 있고, 그때 사용자는 자기가 언제 옮겼는지 기억하지 못한 채 "버튼이 왜 여기 있지" 를 겪는다.
- **창이 작아지면 좌표를 다시 화면 안으로 가둔다.** 안 그러면 버튼이 보이지 않는 곳에 남아
  챗봇을 열 방법이 사라진다.
...
- **수입 `#10B981` · 지출 `#EF4444`** — 이 둘은 액센트 규칙의 예외다. 금액의 방향을 색으로 구분하는 것이 이 앱의 핵심 정보이기 때문이다
  > ⚠️ **일별 캘린더의 금액만 이 규칙에서 빠진다.** 칸마다 배경 농도가 달라 두 색이 4.5:1 을
  > 못 넘긴다. 거기서는 잉크색 + 부호(`+`/`-`)를 쓴다(§3). **다른 화면으로 번지지 않게 한다** —
  > 흰 카드 위에서는 초록·빨강이 제 대비를 내고, 색으로 훑는 것이 실제로 도움이 된다
- 카테고리 팔레트 (색 미지정 시 순서대로 배정):
  `#EF4444 #F59E0B #10B981 #4F46E5 #EC4899 #14B8A6 #8B5CF6 #F97316 #737373`

#### 액센트 밖에 서 있는 전용 토큰 셋

액센트 하나 규칙을 지키되, **다른 색을 빌려 쓰면 그 색의 뜻이 오염되는 자리** 셋만 따로 둔다.
전부 `globals.css` 에 라이트·다크 두 값으로 정의하고, 그 용도 밖에서는 쓰지 않는다.

| 토큰 | 라이트 | 다크 | 쓰는 곳 |
|---|---|---|---|
| `--logo-ring` | `#2563EB` | `#3B82F6` | 로고 마크의 순환 화살표 링 |
| `--heat` · `--heat-scale` | `#A855F7` · `1` | `#C084FC` · `1.5` | 일별 캘린더 칸 배경 농도 |
| `--grid` · `--grid-label` | `#E5E5E5` · `#737373` | `#666666` · `#C9C9C9` | 일별 캘린더 격자선 · 일자/요일 라벨 |

- **`--logo-ring` 은 액센트(243°)와 22° 떨어뜨린 파랑이다.** 액센트를 그대로 쓰면 헤더의 로고가
  버튼·링크와 같은 색이 되어 "브랜드" 와 "누를 수 있는 것" 이 섞인다. 버튼·링크·차트에 쓰지 않는다.
  > ⚠️ **어둡고 진한 파랑을 고르지 않는다.** 흰 배경 대비만 보면 좋아지지만 다크에서 무너진다 —
  > `#3730A3`(indigo-800)은 흰 배경 9.93:1 인데 다크 카드(`#171717`) 위에서 1.80:1 이라 거의 안 보인다.
  > **라이트와 다크 양쪽을 같이 재고 고른다.**
  > ⚠️ **파비콘(`src/app/icon.svg`)에 같은 색이 하드코딩돼 있다.** SVG 파일은 CSS 토큰을 읽지 못하므로
  > 한쪽만 바꾸면 헤더 로고와 브라우저 탭 아이콘의 색이 갈라진다. **항상 같이 바꾼다.**
  > ⚠️ **SVG 는 XML 이라 주석 안에 하이픈 두 개(`--`)를 쓸 수 없다.** 토큰 이름을 주석에 그대로 적으면
  > 파서 오류로 아이콘이 통째로 깨진다. `tsc` 도 `eslint` 도 `.svg` 를 보지 않아 **렌더해야만 드러난다.**
- **`--heat` 는 캘린더 배경 전용이다.** 이 앱의 다른 색은 전부 뜻을 갖고 있어 배경으로 쓸 수 없다 —
  빨강은 지출, 초록은 수입, 인디고는 "누를 수 있음" 이다. 그중 하나로 칸을 칠하면 배경이 데이터나
  조작을 흉내 낸다. 보라 중에서도 **자주빛(271°)** 을 쓴다. 인디고 쪽 보라(`#8B5CF6`, 258°)는
  액센트(243°)와 15° 밖에 안 떨어져 "액센트를 흐리게 칠한 것" 으로 보인다.
- **`--grid` 를 `--border` 와 분리한 이유는 다크에서 요구가 다르기 때문이다.** 카드 테두리는
  "면이 여기서 끝난다" 는 힌트라 흐려도 되지만, 달력의 선은 **어느 칸이 어느 날인지를 말하는 구조**다.
  다크에서 `--border`(`#262626`)는 카드(`#171717`) 위 1.19:1 이라 칸 경계가 뭉개진다.
  라이트 값은 기존과 같아 **달라지는 것은 다크뿐이다.**
...
- **데스크톱에서는 창 크기를 바꿔도 본문 폭이 따라 변하지 않는다.** `body { min-width: 64rem }` 을
  `md`(48rem) 이상에서만 건다. 네이버처럼 데스크톱 레이아웃을 한 번 고정하고, 좁아지면 가로
  스크롤을 내주는 방식이다.
  > ⚠️ **`md` 미만에는 걸지 않는 것이 요점이다.** 전역으로 걸면 모바일에서 1024px 짜리 본문이
  > 되어 반응형이 통째로 죽는다.
- **`html { scrollbar-gutter: stable }` 을 건다.** 스크롤바가 생기고 사라질 때마다 본문이 좌우로
  흔들리는 것을 막는다. 목록이 짧은 화면과 긴 화면을 오갈 때 드러난다.
  > ⚠️ **폭을 잴 때는 `window.innerWidth` 가 아니라 `document.documentElement.clientWidth` 를 쓴다.**
  > 앞의 것은 스크롤바를 포함해, 자리를 비워 둔 만큼(보통 15px) 실제 본문 폭과 어긋난다.
  > `position: fixed` 요소의 좌표계도 스크롤바를 제외하므로 같은 기준으로 비교해야 한다.
