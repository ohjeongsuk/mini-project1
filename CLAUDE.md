...
| 일별 지출 세로 막대 | `div` 높이를 `%`로 | 〃. 일별 지출은 이산값이라 선보다 정확하다 |
...
- **일별 캘린더는 칸마다 그날의 수입·지출을 적고, 누르면 그날 내역으로 간다.**
  ...
  금액이 없는 날도 `0원` 을 적는다. 빈칸으로 두면 "기록이 없다" 와 "0원" 이 구분되지 않는다.
  ⚠️ **칸이 좁아 `formatAmountShort` 로 줄이고 0 이 아닐 때는 `원` 을 뗀다**(데스크톱 칸 안쪽 49px,
  `10.7만원` 은 58px). 정확한 값은 `title`·`aria-label` 이 원 단위로 준다.
  ⚠️ **배경 농도 상한은 0.15 다.** 칸에 빨간 지출 글자가 얹히므로 예전 상한 1 로 두면
  빨강 위의 빨강이 되어 대비가 2.5:1 까지 떨어진다.
- **일별 지출 추이는 선·막대·누적 셋 중 고를 수 있다.** 선택은 `localStorage`(`moneylog_chart`)에 남는다.
  월 이동(`?ym=`)이 URL 인 것과 다르다 — 월은 "무엇을 보는가" 이고 그래프 종류는 "어떻게 보는가" 라
  링크로 공유할 값이 아니다. 누적은 새 컴포넌트 없이 `TrendLine` 에 누적합을 넘겨 그린다.
- **세로축 눈금은 `ChartAxis` 한 곳에서 만든다.** 선과 막대가 같은 축을 써야
  그래프 종류만 바꿨을 때 눈금이 흔들리지 않는다. 라벨은 만·억으로 축약한다(`formatAmountShort`) —
  누적은 축 최댓값이 한 달 총액이라 `1,000,000원` 이 라벨 칸(72px)을 2px 넘긴다.
